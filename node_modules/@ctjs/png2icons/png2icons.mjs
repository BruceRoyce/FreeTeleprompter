...
function encodeIconWithPackBits(image, withAlphaChannel) {
    const R = encodeWithPackBitsForICNS(getImageChannel(image, 4, 0));
    const G = encodeWithPackBitsForICNS(getImageChannel(image, 4, 1));
    const B = encodeWithPackBitsForICNS(getImageChannel(image, 4, 2));
    if (withAlphaChannel) {
        const header = Buffer.alloc(4);
        header.write("ARGB", 0, 4, "ascii");
        const A = encodeWithPackBitsForICNS(getImageChannel(image, 4, 3));
 ...
 */
export function createICNS(input, scalingAlgorithm, numOfColors) {
 ...
 */
export function createICO(input, scalingAlgorithm, numOfColors, PNG, forWinExe) {
    // Handle caching of input.
    checkCache(input);
    // Source for all resizing actions
    let inputImage = getImageFromPNG(input);
